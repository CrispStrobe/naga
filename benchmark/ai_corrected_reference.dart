  ...
  Point<int> _move(Point<int> p, Direction dir) {
    ...
    final y =
        ...
        (dir == Direction.down
            ...
            ? -1
            : 0);
    return _wallsKill ? Point(x, y) : Point(x % _width, y % _height);
  }
  ...
  Direction _decideExpert(
    ...
    if (target != null) {
      ...
      if (foodDir != null && safe.contains(foodDir)) {
        ...
        final tailTarget = segments.length == 1
            ? next
            : willEat
            ? tail
            : segments[segments.length - 2];
        simOccupied.remove(tailTarget);
    ...
    for (final d in safe) {
      ...
      // The new head is the flood-fill origin, not an impassable obstacle.
